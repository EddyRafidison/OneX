...
async function signup (req, res) {
  try {
    ...
    if (!geo || geo.country !== 'MG')
      return res.json({ msg: 'unsupported country' })
    ...
    // Vérifier unicité
    const exists = await pool.promiseQuery(
      'SELECT 1 FROM auths WHERE username=?',
      [username]
    )
...
async function signin (req, res) {
  ...
  try {
    ...
    const rows = await pool.promiseQuery(
      'SELECT password,seed,status,id FROM auths WHERE username=?',
      [User]
    )
    if (!rows.length) return res.json({ msg: 'error', ua: '' })
    const [encPwd, encSeed, status, id] = [
      rows[0].password,
      rows[0].seed,
      rows[0].status,
      rows[0].id
    ...
    if (status === 0) {
      await pool.promiseQuery('UPDATE auths SET status=1 WHERE username=?', [
        User
      ])
...
async function userLastStock (req, res) {
  ...
  try {
    ...
    const [{ balance }] = await pool.promiseQuery(
...
async function nearTransfer (req, res) {
  ...
  try {
    ...
    const [{ status: stS }] = await pool.promiseQuery(
      'SELECT status FROM auths WHERE username=?',
      [S]
    )
    if (stS < 1) throw 0
    // Récupérer prix courant
    const [{ unit_price: p }] = await pool.promiseQuery(
      'SELECT MAX(unit_price) AS unit_price FROM common',
      []
    )
    ...
    const [{ balance: balS }] = await pool.promiseQuery(
      'SELECT balance FROM users_stock WHERE username=?',
      [S]
    )
    if (new BigNumber(balS).lt(minReqBal))
      return res.json({ transf: 'insufficient balance' })
    // Check dest exists & status
    const [{ status: stD }] = await pool.promiseQuery(
      'SELECT status FROM auths WHERE username=?',
      [D]
    )
    if (stD < 1) throw 0
    // Check stock max
    const [{ balance: balD }] = await pool.promiseQuery(
      'SELECT balance FROM users_stock WHERE username=?',
      [D]
    )
    if (
      new BigNumber(balD).plus(Amount).multipliedBy(p).gt(MAX_STOCK_DEFAULT)
    ) {
    ...
    const [{ total_units_price: tp, backed_units: bu }] =
      await pool.promiseQuery(
        'SELECT * FROM common ORDER BY id DESC LIMIT 1',
        []
      )
...
async function modifyPwd (req, res) {
  ...
  const U = user.replaceAll(' ', '+'),
    P1 = pswd1.replaceAll(' ', '+'),
    P2 = pswd2.replaceAll(' ', '+')
  if (!verifyUA(req, U, P1, tkn))
    return res.json({ auth: 'incorrect auth or forbidden request' })

  try {
    await pool.promiseQuery('UPDATE auths SET password=? WHERE username=?', [
      encrypt(P2, process.env.DB_PSWD),
      U
    ])
...
async function deleteUser (req, res) {
  ...
  const U = user.replaceAll(' ', '+'),
    P = pswd.replaceAll(' ', '+')
  if (!verifyUA(req, U, P, tkn))
    return res.json({ auth: 'incorrect auth or forbidden request' })

  try {
    // Valeur du compte
    const [{ unit_price: p }] = await pool.promiseQuery(
      'SELECT MAX(unit_price) AS p FROM common',
      []
    )
    const [{ balance: b }] = await pool.promiseQuery(
      'SELECT balance FROM users_stock WHERE username=?',
      [U]
    )
...
async function recoverAccount (req, res) {
  ...
  const U = user.replaceAll(' ', '+'),
    S = seed.replaceAll(' ', '+')
  try {
    const [{ seed: encSeed }] = await pool.promiseQuery(
      'SELECT seed FROM auths WHERE username=?',
      [U]
    )
